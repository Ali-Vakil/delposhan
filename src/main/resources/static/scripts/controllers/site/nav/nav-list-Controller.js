app.controller('navListCtrl', function ($scope, ApiHandler, $rootScope) {
    ...
    $scope.dataList = [];
    $scope.getdatalist = () => {
        let url = 'Nav/getAll?pageSize=' + $scope.qurey.pageSize + '&pageNumber=' + $scope.qurey.pageNumber;
    ...
    $scope.changeOrder = (id, navigationId) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "You won't be able to revert this!",
            icon: 'warning',
            ...
            confirmButtonText: 'Yes, Do it!'
                ...
                if (result.isConfirmed) {
                    ApiHandler.callPost('Nav/updateOrder/' + id + '/' + navigationId,null, (response) => {
                        Swal.fire(
                            'Do it!',
